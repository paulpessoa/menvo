-- =================================================================
-- MENVO - CONSOLIDATED AUTHENTICATION SCHEMA
-- This script creates the complete authentication system from scratch
-- Replaces all previous conflicting scripts with a unified approach
-- =================================================================
...
BEGIN
    ...
    -- Mentor permissions
    INSERT INTO public.role_permissions (role_id, permission_id) 
    SELECT mentor_role_id, id FROM public.permissions 
    WHERE name IN ('view_mentors', 'view_profiles', 'update_own_profile', 'provide_mentorship', 'manage_availability');
    
    -- Mentee permissions
    INSERT INTO public.role_permissions (role_id, permission_id) 
    SELECT mentee_role_id, id FROM public.permissions 
    WHERE name IN ('view_mentors', 'view_profiles', 'update_own_profile', 'book_sessions');
    
    -- Moderator permissions
    INSERT INTO public.role_permissions (role_id, permission_id) 
    SELECT moderator_role_id, id FROM public.permissions 
    WHERE name IN ('view_mentors', 'view_profiles', 'moderate_content', 'moderate_verifications', 'admin_verifications');
    
    -- Volunteer permissions
    INSERT INTO public.role_permissions (role_id, permission_id) 
    SELECT volunteer_role_id, id FROM public.permissions 
    WHERE name IN ('view_mentors', 'view_profiles', 'validate_activities');
END $;
...
-- =================================================================
-- 7. CREATE MAIN TRIGGER FUNCTION FOR USER REGISTRATION
-- =================================================================
...
-- =================================================================
-- 8. CREATE JWT CUSTOM ACCESS TOKEN HOOK
-- =================================================================
...
-- =================================================================
-- 11. CONFIGURE ROW LEVEL SECURITY (RLS)
-- =================================================================
...
SELECT 'CONSOLIDATED AUTHENTICATION SCHEMA CREATED SUCCESSFULLY!' as status;
...
SELECT 
    'Functions created: ' || COUNT(*) as summary
FROM pg_proc p
...
SELECT 
    'Triggers created: ' || COUNT(*) as summary
FROM pg_trigger t
JOIN pg_class c ON t.tgrelid = c.oid
JOIN pg_namespace n ON c.relnamespace = n.oid
WHERE n.nspname = 'public'
AND t.tgname LIKE '%auth%' OR t.tgname LIKE '%profile%' OR t.tgname LIKE '%updated_at%';

-- Show role and permission counts
SELECT 
    'Roles: ' || (SELECT COUNT(*) FROM public.roles) || 
    ', Permissions: ' || (SELECT COUNT(*) FROM public.permissions) ||
    ', Role-Permission mappings: ' || (SELECT COUNT(*) FROM public.role_permissions) as rbac_summary;
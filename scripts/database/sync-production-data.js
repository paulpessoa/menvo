...
async function exportProductionData() {
  ...
  try {
    ...
    try {
      logStep("Exporting mentor availability_status...")
      ...
      logSuccess(
        `Exported ${mentorAvailability.length} mentor availability_status records`
      )
    } catch (error) {
...
async function importToLocal(exportFile) {
  ...
  try {
    ...
    const clearConfirm = await askQuestion(
      "⚠️  This will clear existing local data. Continue? (y/N): "
    )
    ...
    // Clear mentor availability_status
    ...
    for (const authUser of exportData.auth_users) {
      try {
        ...
        if (error) {
          logWarning(
            `Failed to import user ${authUser.email}: ${error.message}`
          )
        } else {
          importedUsers++
        }
    ...
    logSuccess(
      `Imported ${importedUsers}/${exportData.auth_users.length} auth users`
    )
    ...
    if (exportData.user_roles && exportData.user_roles.length > 0) {
      ...
      if (userRolesError) {
        ...
        logSuccess(
          `Imported ${exportData.user_roles.length} user role assignments`
        )
      }
    }

    // Import mentor availability_status
    if (
      exportData.mentor_availability &&
      exportData.mentor_availability.length > 0
    ) {
      logStep("Importing mentor availability_status...")
      ...
      if (mentorError) {
        logWarning(
          `Mentor availability_status import warning: ${mentorError.message}`
        )
      } else {
        logSuccess(
          `Imported ${exportData.mentor_availability.length} mentor availability_status records`
        )
      }
    }
...
async function validateDataIntegrity() {
  ...
  try {
    ...
    const profilesWithoutAuth = profiles.filter(
      (p) => !users.users.find((u) => u.id === p.user_id)
    )

    const rolesWithoutAuth = userRoles.filter(
      (r) => !users.users.find((u) => u.id === r.user_id)
    )

    if (profilesWithoutAuth.length > 0) {
      logWarning(
        `Found ${profilesWithoutAuth.length} profiles without auth users`
      )
    }

    if (rolesWithoutAuth.length > 0) {
      logWarning(
        `Found ${rolesWithoutAuth.length} role assignments without auth users`
      )
    }
    ...
    if (profiles.length > 0) {
      ...
      profiles.slice(0, 5).forEach((profile) => {
        ...
        const roleInfo = hasRole
          ? ` (${hasRole.role || "role assigned"})`
          : " (no role)"

...
- **Not done, needs the founder:** confirm `AI_METERING_KEY` is set in Vercel and matches the hash in `private.ai_settings` (migration `20260923000003`'s header has the setup steps) — without it, AI cost metering rows are silently rejected while requests still work. Regenerate `lib/types/supabase.ts` via `npm run db:types` (tables/RPCs from `20260923000002`–`20260923000006` were still hand-patched or missing from generated types). Consider adding a migration for `ai_assistant_flag` (currently `true` in the `feature_flags` table but created manually, no migration file — same gap `chat_flag` already covers correctly).

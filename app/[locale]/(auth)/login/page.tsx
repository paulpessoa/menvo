...
export default function LoginPage() {
  ...
  const { signIn, signInWithProvider, signInWithGoogleNew, user, role, loading, getDefaultRedirectPath } = useAuth()
  ...
  const handleGoogleNew = async () => {
    setIsSocialLoading("google")
    setError("")
    try {
      await signInWithGoogleNew()
    } catch (err: any) {
      setError(err.message || t("error.loginFailed"))
    } finally {
      setIsSocialLoading(null)
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            ...
            <Button
              type="button"
              variant="default"
              className="w-full flex items-center justify-center gap-3 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold"
              onClick={handleGoogleNew}
              disabled={isSocialLoading === "google"}
            >
              {isSocialLoading === "google" ? (
                <Loader2 className="h-4 w-4 animate-spin" />

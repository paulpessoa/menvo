...
export default function AdminSuggestionsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "approved": return <Badge variant="secondary" className="bg-green-100 text-green-800 border-green-200">Aprovado</Badge>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">Sugestões de Temas</h1>
              <p className="text-muted-foreground">O que os usuários estão procurando e não encontram</p>
            </div>
          </div>
                ...
                suggestions.map((s) => (
                  <Card key={s.id} className="overflow-hidden">
                    ...
                    <CardContent className="pt-4 space-y-4">
                      <div>
                        <p className="text-sm font-semibold mb-1">O que procura:</p>
                        <p className="text-sm bg-blue-50 p-3 rounded-md border border-blue-100 italic">
                          "{s.suggestion_text}"
                        </p>
                      </div>
                      ...
                      {(s.free_topics?.length > 0 || s.inclusion_tags?.length > 0) && (
                        <div className="flex flex-wrap gap-2">
                          {s.free_topics?.map((t: string) => (
                            <Badge key={t} variant="outline" className="bg-purple-50">{t}</Badge>
                          ))}
                          {s.inclusion_tags?.map((t: string) => (
                            <Badge key={t} variant="outline" className="bg-pink-50 border-pink-200">{t}</Badge>
                          ))}
                        </div>
                      )}

    ...
    try {
      ...
      const totalMinutes =
        appointments
          ...
          .reduce((sum: number, apt: any) => sum + (apt.duration_minutes || 0), 0) || 0
      const totalHours = Math.round((totalMinutes / 60) * 10) / 10

      const uniqueMentors = new Set(appointments.map((apt: any) => apt.mentor_id))
        .size
  ...
  const fetchUpcomingAppointments = async () => {
    try {
      ...
      const formattedAppointments =
        data?.map((apt) => {
          const mentorProfile = Array.isArray(apt.profiles) ? apt.profiles[0] : apt.profiles;
          return {
            id: apt.id,
                ...
                <Card className="hover:shadow-md transition-shadow border-l-4 border-l-green-500">
                  ...
                  <CardContent>
                    <p className="text-sm text-muted-foreground mb-4">
                      {t("mentee.actions.myMentorshipsDesc")}
                    </p>
                    <Button
                      asChild
                      variant="outline"
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {t("mentee.sections.favoritesDesc")}
                    </CardDescription>
                  </div>
                ...
                <Card>
                  <CardContent className="py-8 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t("mentee.sections.noSessions")}
                    </p>
                    <Button asChild variant="link" className="mt-2">
                      <Link href="/mentors">
                        {t("mentee.sections.bookNow")}
                      </Link>
                    </Button>
                  </CardContent>
                </Card>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center gap-3 mb-3">
                          <Avatar className="h-8 w-8">
                            <AvatarImage
                              src={appt.mentor.avatar_url || undefined}
                            />
                            <AvatarFallback>
                              {appt.mentor.full_name[0]}
                            </AvatarFallback>
                          </Avatar>
                          <div>
                            <p className="text-sm font-medium">
                              {appt.mentor.full_name}
                            </p>
                            <p className="text-xs text-muted-foreground">

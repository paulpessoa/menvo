...
export default function MentorProfilePage({ params }: MentorProfilePageProps) {
  ...
  const handleShare = async () => {
    try {
      await navigator.share({
        ...
        text: mentor.bio || "Conheça este mentor!",
  ...
  const formatExperience = () => {
    if (!mentor.experience_years) return null
    return `${mentor.experience_years} ${mentor.experience_years === 1 ? "ano" : "anos"} de experiência`
  }
        ...
        <div className="space-y-8">
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1 space-y-3">
              ...
              <div className="flex flex-wrap gap-y-2 gap-x-4 text-muted-foreground text-sm">
                {mentor.job_title && (
                  <div className="flex items-center gap-1">
                    ...
                    <span>
                      {mentor.job_title}{" "}
                      {mentor.company && ` na ${mentor.company}`}
                    </span>
                  </div>
                )}
                {mentor.location && (
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                ...
                >
                  <availabilityStatus.icon className="h-3 w-3 mr-1" />{" "}
                  {availabilityStatus.text}
                </Badge>
                {(mentor.mentor_skills || []).slice(0, 3).map((s: string) => (
          ...
          <Tabs defaultValue="about" className="w-full">
            <TabsList className="grid w-full grid-cols-3 md:w-auto">
              ...
              <TabsTrigger value="availability_status">
                Disponibilidade
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="expertise" className="pt-4">
              <div className="flex flex-wrap gap-2">
                {(mentor.mentor_skills || []).map((s: string) => (
                  <Badge
                    key={s}
                    variant="secondary"
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Reserve uma sessão gratuita de 45 min
              </CardDescription>
            </CardHeader>

...
export async function analyzeCV(
  ...
  const { provider = "mock", language = "pt", minConfidence = 0.7 } = options
 ...
 */
export function mapAnalysisToProfileFields(
  ...
  if (
    ...
    fieldConfidence.currentPosition >= minConfidence
  ) {
    profileUpdates.job_title = extractedData.currentPosition
  }

  if (
    extractedData.currentCompany &&
    fieldConfidence.currentCompany >= minConfidence
  ) {
    profileUpdates.company = extractedData.currentCompany
  }
  ...
  if (extractedData.location) {
    if (extractedData.location.city)
      profileUpdates.city = extractedData.location.city
    if (extractedData.location.state)
      profileUpdates.state = extractedData.location.state
    if (extractedData.location.country)
      profileUpdates.country = extractedData.location.country
  }
  ...
  if (
    extractedData.mentorshipTopics &&
    extractedData.mentorshipTopics.length > 0
  ) {
...
export async function storeCVMetadata(
  ...
  const metadata = {
    ...
    extracted_certifications:
      analysisResult?.extractedData.certifications || null,

...
export function validateOAuthProvider(
  ...
  if (
    provider === "linkedin" &&
    config.clientSecret &&
    !config.clientSecret.startsWith("WPL_AP1.")
  ) {
...
/**
 * Test OAuth provider availability_status
 */
...
> {
  ...
  for (const provider of providers) {
    const validation = validateOAuthProvider(provider)
    const test = validation.isValid
      ? await testOAuthProvider(supabase, provider)
      : null

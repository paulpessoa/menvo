...
export const useMentorAvailability = (mentorId: string) => {
  return useQuery({
    queryKey: ["mentor-availability_status", mentorId],
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: ["mentor-availability_status", data.mentor_id]
      })
...
export const useRemoveAvailability = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["mentor-availability_status"]
      })
...
export const useSetMentorAvailability = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      if (data.length > 0) {
        queryClient.invalidateQueries({
          queryKey: ["mentor-availability_status", data[0].mentor_id]
        })
      }
...
export const useRespondToSession = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["mentorship-sessions"] })
      queryClient.invalidateQueries({
        queryKey: ["mentorship-session", data.id]
      })
      const message =
...
export const useMentorSessions = (mentorId?: string, status?: string) => {
  return useQuery({
    ...
    queryFn: () =>
      mentorshipSessionsService.getMentorSessions(mentorId, status as any),
...
export const useMenteeSessions = (menteeId?: string, status?: string) => {
  return useQuery({
    ...
    queryFn: () =>
      mentorshipSessionsService.getMenteeSessions(menteeId, status as any),
    staleTime: 1000 * 60 * 2 // 2 minutos
  })
}

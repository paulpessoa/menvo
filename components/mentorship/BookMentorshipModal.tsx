    ...
    try {
      ...
      if (user) {
        ...
        if (completed && completed.length > 0) {
          const { data: feedbacks } = await (
            supabase.from("appointment_feedbacks") as any
          )
            .select("appointment_id")
            .eq("reviewer_id", user.id)

          const feedbackIds = new Set(
            (feedbacks as any[])?.map((f: any) => f.appointment_id)
          )
      ...
      // Usar a API de availability_status que já filtra horários ocupados
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto p-0 border-none">
        <div className="bg-white rounded-lg overflow-hidden flex flex-col h-full">
          <DialogHeader className="p-6 border-b bg-gray-50">
            ...
            <DialogDescription>
              Escolha um dos horários disponíveis abaixo para solicitar sua
              mentoria.
            </DialogDescription>
          </DialogHeader>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="text-sm text-muted-foreground animate-pulse text-center">
                  Buscando horários disponíveis do mentor...
                </p>
              </div>
            ) : pendingEvaluation ? (
              <div className="py-12 text-center space-y-4">
                ...
                <p className="text-gray-600 max-w-sm mx-auto">
                  ...
                  Por favor, avalie sua última sessão para liberar novos
                  agendamentos.
                </p>
                <Button asChild className="mt-6">
                  <Link href="/mentorship/mentee">Ver minhas mentorias</Link>
                </Button>
              </div>
            ) : error ? (
              <div className="py-12 text-center space-y-4">
              ...
              <div className="space-y-6">
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <p className="text-sm text-blue-800 leading-relaxed">
                    As mentorias do <strong>{mentorName}</strong> têm duração
                    padrão de <strong>45 minutos</strong>. Selecione um horário
                    abaixo para prosseguir.
                  </p>
                </div>
                ...
                {availableSlots.length === 0 ? (
                  <div className="text-center py-16 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                    ...
                    <p className="text-sm text-gray-500 mt-2 max-w-xs mx-auto">
                      Não encontramos horários livres nas próximas 2 semanas.
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center px-1">
                    <p
                      className={`text-[10px] font-medium ${message.length < 20 ? "text-red-500" : "text-green-600"}`}
                    >
                ...
                {success && (
                  <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm font-bold flex items-center gap-2">
                    <CheckCircle className="w-5 h-5" /> Solicitação enviada!
                    Redirecionando...
                  </div>
                )}

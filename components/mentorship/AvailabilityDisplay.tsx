...
export default function AvailabilityDisplay({
  ...
  const [groupedAvailability, setGroupedAvailability] = useState<
    Record<number, AvailabilitySlot[]>
  >({})
  ...
  useEffect(() => {
    ...
    const grouped = availability_status.reduce(
      (acc, slot) => {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>{t("display.cardDescription")}</CardDescription>
      </CardHeader>
      <CardContent>{renderDetailedView()}</CardContent>
    </Card>
  )
}

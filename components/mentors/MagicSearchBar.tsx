  ...
  const handleMagicSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.no_match) {
        ...
        onMatch(result.suggestions, result.global_justification, query.trim(), result.suggested_topics || [])
        setHasResult(true)
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mb-8 sm:mb-10">
      <form onSubmit={handleMagicSearch} className="relative group">
        <div className="absolute -inset-1 bg-gradient-to-r from-primary to-primary-700 rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-1000 group-hover:duration-200" />
